...
**Do not use `{ optional: true }`** unless you explicitly want users to allowlist the tool. Optional tools are never auto-enabled — they are hidden from all agents until added to an allowlist in `openclaw.json`. Omit the second argument to make tools available by default.
...
| Tools registered but not visible to agents | `{ optional: true }` prevents auto-enablement | Remove `optional: true` or add tools to agent allowlist in `openclaw.json` |
